...
const systemSteps = [
  {
    ...
    body: "Interfaces, product logic, web, mobile, prototypes, and the decisions that make the whole thing coherent.",
  },
  {
    key: "connect",
    label: "CONNECT",
    title: "Make the pieces behave like one system.",
    body: "APIs, cloud services, databases, auth, and data flows connected without creating another mess.",
  },
  {
    key: "automate",
    label: "AUTOMATE",
    title: "Give repetitive work back to software.",
    body: "AI when it earns its keep, deterministic automation when it does not, and human judgment where it matters.",
  },
];
...
const projectFlows: Record<VisualKind, { eyebrow: string; nodes: string[]; note: string }> = {
  spenthere: {
    eyebrow: "ONE RECEIPT / MORE USEFUL CONTEXT",
    nodes: ["RECEIPT", "STRUCTURE", "LOCAL FACTS"],
    note: "Price, parking, fees, party context — extracted without turning contribution into homework.",
  },
  coordinator: {
    eyebrow: "CONTROLLED WRITE PATH",
    nodes: ["AI PROPOSES", "HUMAN APPROVES", "PROJECT UPDATES"],
    note: "The automation is useful because the permission boundary is part of the product, not an afterthought.",
  },
  credentials: {
    eyebrow: "ONE-TIME SECRET FLOW",
    nodes: ["REQUEST", "EXPIRING LINK", "CONTROLLED VAULT"],
    note: "Ask for exactly what is needed, transfer it once, and stop passing passwords around casually.",
  },
};

function ProjectFlow({ kind }: { kind: VisualKind }) {
  ...
  return (
    <div className="relative flex min-h-[250px] flex-col justify-center py-6 lg:pl-8">
      ...
      <p className="mt-11 max-w-xl text-sm leading-6 text-white/40">{flow.note}</p>
    </div>
  );
}
        ...
        >
          <div>
            ...
            >
              Build the thing.
              <br />
              <span className="text-white/28">Connect the things.</span>
              <br />
          ...
          <HeroFlow activeSystem={activeSystem} setActiveSystem={setActiveSystem} />
        </div>
      </section>
            ...
            <div className="border-t border-black/15">
              {capabilities.map((capability, index) => (
                <div key={capability.title} className="grid gap-5 border-b border-black/15 py-9 md:grid-cols-[48px_1fr] md:gap-8">
                  ...
                  <div>
                    <h3 className="text-3xl font-bold tracking-[-0.035em] md:text-4xl">{capability.title}</h3>
                    <p className="mt-4 max-w-2xl text-base leading-7 text-black/55">{capability.body}</p>
                  </div>
                </div>
              ))}
            </div>
